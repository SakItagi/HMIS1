...
const HRDashboard = ({ chartType = 'overview' }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Ensure full content is rendered before capture
    pdfRef.current.scrollIntoView({ behavior: 'auto', block: 'start' });
    await new Promise((res) => requestAnimationFrame(() => res()));
    await new Promise((res) => setTimeout(res, 500));
    pdfRef.current.scrollTop = pdfRef.current.scrollHeight;
    await new Promise((res) => setTimeout(res, 500));
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: '18px', color: 'black' }}>
          Workforce trends show monthly shifts in joinees and resignations.
          While joinee counts met expectations, higher resignation rates suggest possible retention issues.
        </p>
      </div>
    </div>
  );
