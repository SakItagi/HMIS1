...
const MaintenanceDashboard = ({ view = 'overview' }) => {
  ...
  const dashboardRef = useRef(null); // For PDF export
  ...
  return (
    <div style={{ padding: '10px', fontFamily: 'Futura, Arial, sans-serif', position: 'relative' }}>
      {/* Download PDF Button with Absolute Positioning */}
      ...
      <div ref={dashboardRef}>
        {view === 'expected-vs-actual' ? (
          <>
            <h1 style={{ color: 'black', fontSize: '22px', marginBottom: '10px', fontWeight: 'normal' }}>
              Expected vs Actual: Repair & Purchase Costs
            </h1>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
          ...
          <tbody>
            {(view === 'overview'
              ? ['RepairCost', 'PurchaseCost']
              : ['ExpectedRepairCost', 'RepairCost', 'ExpectedPurchaseCost', 'PurchaseCost']
            ).map(key => (
              <tr key={key}>
        ...
        <p style={{ fontSize: '18px', color: 'black' }}>
          Maintenance trends indicate fluctuations in repair and purchase costs over time. While purchase costs were mostly predictable, repair costs occasionally exceeded expectations, suggesting possible equipment failures or urgent maintenance issues in certain months.
        </p>
      </div>
    </div>
  );
