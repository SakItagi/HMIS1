...
const FeedbackDashboard = ({ chartType = 'overview' }) => {
      ...
      try {
        ...
        raw.forEach(item => {
          ...
          Object.entries(item).forEach(([key, val]) => {
            ...
            cleanItem[cleanKey] = typeof val === 'string' ? val.replace(/\r/g, '').trim() : val;
          });
        ...
        const finalData = Object.values(summary).sort((a, b) => {
          const getMonthIndex = (m) => {
            ...
            return ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
              'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].indexOf(name);
          };
      ...
      <>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <Tooltip formatter={(val) => `${Math.round(val)}`} />
            <Legend />
            <Line type="linear" dataKey="IPDScore" stroke="#1E88E5" name="IPD Score" strokeWidth={2} label={renderLineLabel} />
            <Line type="linear" dataKey="OPDScore" stroke="#D32F2F" name="OPD Score" strokeWidth={2} label={renderLineLabel} />
          </LineChart>
        </ResponsiveContainer>
      </>
    );
  ...
  return (
    <div ref={dashboardRef} style={{ padding: '10px', fontFamily: 'Futura, Arial, sans-serif', position: 'relative' }}>
      {/* Updated PDF Button with consistent styling */}
      ...
      <p style={{ fontSize: '18px', color: 'black' }}>
        ...
        Observed trends show general alignment with expectations, though OPD scores occasionally fluctuate due to varying patient volumes or services rendered.
      </p>
    </div>
  );
};
